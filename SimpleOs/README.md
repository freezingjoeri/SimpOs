...
2. Haal SimpOs binnen en start het install‑script  
   ...
   # 2) Naar de projectmap gaan (binnen de repo staat nog een map 'SimpOs')
   cd SimpOs/SimpOs    # of cd simpos/SimpOs, afhankelijk van de mapnaam

   # 3) Install‑script draaien
   bash install_linux.sh
   ```

   - Als `cd SimpOs/SimpOs` een fout geeft, doe dan eerst `ls` om de exacte
     mapnaam te zien en gebruik die (bijv. `simpos/SimpOs`).
   - Het script wordt uitgevoerd in de juiste projectmap en detecteert zelf
     waar SimpOs staat.
...
```bash
cd SimpOs/SimpOs   # of de map waar je project staat
git pull --ff-only
```
