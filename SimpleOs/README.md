...
1. Open een terminal in de map waar `main.py` staat (bijv. `SimpleOs`).
...
2. Haal SimpOs binnen en start het install‑script  
   ...
   # 2) Naar de map gaan die net is gemaakt
   cd SimpOs
   ...
   - Als `cd SimpOs` een fout geeft, doe dan eerst `ls` om de exacte mapnaam te zien
     en gebruik die.
   - Het script wordt altijd uitgevoerd in de **map waar het in staat**, dus je hoeft
     geen pad meer met `~/SimpleOs` of `~/SimpOs` te raden.
...
2. Het script:
   ...
   - zet auto‑start in `~/.bash_profile` of `~/.profile`:
     ...
     ```bash
     cd "$HOME/SimpleOs" && source .venv/bin/activate && python main.py
     ```
...
```bash
cd SimpOs   # of de map waar je repo staat
git pull --ff-only
```
...
- `help`, `status`, `ai`, `netstat`, `whoami`, `admin`, `settings`, `reboot`, `shutdown`, enz.
...
2. Install Python and copy this `SimpleOs` folder into the VM.
3. Configure the system to auto-start SimpOs after boot, for example by:
   - Adding `python /path/to/SimpleOs/main.py` to `.bash_profile` or
